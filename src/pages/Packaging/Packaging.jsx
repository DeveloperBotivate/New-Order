...
import { getReceivedOrders, getDispatchHistory, getPackagingHistory } from '../../utils/storageManager';
...
export default function Packaging() {
  ...
  const pendingItems = useMemo(() => {
    return orders.filter(order => {
      // Find all dispatched items for this order
      const orderDispatches = dispatchHistory.filter(d => d.orderId === order.orderId);
      if (orderDispatches.length === 0) return false;
      
      // Check if any of these dispatched items are NOT packaged (Packaging = Yes).
      // Matched by dispatchId — each dispatch transaction (including partial ones)
      // is packaged independently.
      return orderDispatches.some(dispatchItem => {
        const isPackaged = packagingHistory.some(ph =>
          ph.dispatchId === dispatchItem.dispatchId &&
          ph.packagingStatus === 'Yes'
        );
        return !isPackaged;
      });
    }).reverse();
  }, [orders, dispatchHistory, packagingHistory]);
  
  // 2. History Items: Orders that HAVE items successfully packaged
  const historyItems = useMemo(() => {
    return orders.filter(order => {
      return packagingHistory.some(ph => ph.orderId === order.orderId && ph.packagingStatus === 'Yes');
    }).reverse();
  }, [orders, packagingHistory]);
  ...
  return (
    <div className="p-0 sm:p-1 md:p-3 space-y-2 md:space-y-3 flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {activeTab === 'pending' ? (
          <PendingPackaging data={pendingItems} filters={filters} refresh={refreshData} />
        ) : (
          <HistoryPackaging data={historyItems} filters={filters} refresh={refreshData} />
        )}
      </div>
    </div>
  );
