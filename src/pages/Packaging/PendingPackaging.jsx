...
import FormPackaging from './FormPackaging';
import { getDispatchHistory, getPackagingHistory } from '../../utils/storageManager';
import { isPdfDataUrl } from '../../utils/helpers';

export default function PendingPackaging({ data, filters, refresh }) {
  ...
  const tableHeaders = [
    ...
    "Expected Delivery Date", "Transporting Type", "Total Product", "Total PO Value", "Advance Payment", "Advance Amount", "Remarks",
  ...
  const renderRow = (order) => {
    ...
    const orderDispatches = allDispatch.filter(dh => dh.orderId === order.orderId);

    // Filter to only those not yet packaged (matched by dispatchId — each dispatch
    // transaction, including partial ones, is packaged independently)
    const allPackaging = getPackagingHistory() || [];
    const pendingPackagingItems = orderDispatches.filter(dispatchItem => {
      return !allPackaging.some(ph => ph.dispatchId === dispatchItem.dispatchId && ph.packagingStatus === 'Yes');
    });

    const totalProductCount = pendingPackagingItems.length;
                ...
                <div className="bg-white rounded-xl border border-indigo-100 shadow-sm overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {pendingPackagingItems.length === 0 ? (
                        <tr><td colSpan="12" className="px-4 py-3 text-center text-xs text-gray-500">No pending items.</td></tr>
                      ) : (
                        pendingPackagingItems.map((item, idx) => {
