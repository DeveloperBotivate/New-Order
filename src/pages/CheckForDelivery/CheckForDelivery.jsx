...
export default function CheckForDelivery() {
  ...
  return (
    <div className="p-0 sm:p-1 md:p-3 space-y-2 md:space-y-3 flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {activeTab === 'pending' ? (
          <PendingCheckforDelivery data={pendingItems} filters={filters} refresh={refreshData} />
        ) : (
          <HistoryCheckforDelivery data={historyItems} filters={filters} refresh={refreshData} />
        )}
      </div>
    </div>
  );
