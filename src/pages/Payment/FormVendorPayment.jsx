...
export default function FormVendorPayment({ order, onClose, onSubmit }) {
  ...
  const validateForm = () => {
    ...
    if (parseFloat(formData.amountPaid) > order.pendingAmount) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium mt-0.5">Process vendor payment for PO: {order.poNumber}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          <div className="bg-gray-50/80 rounded-xl p-4 border border-gray-100">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-700">₹{(order.totalPaid || 0).toFixed(2)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-8 pr-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500/20 font-medium ${errors.amountPaid ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-gray-50/50'}`}
                    placeholder="Enter amount"
                  />
                </div>
                {errors.amountPaid && <p className="text-[10px] text-red-500 mt-1">{errors.amountPaid}</p>}
                <p className="text-[9px] text-gray-400 mt-1">Maximum allowed: ₹{(order.pendingAmount || 0).toFixed(2)}</p>
              </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 shrink-0 flex justify-end gap-3">
          ...
          <button
            ...
            className="px-6 py-2 rounded-lg font-bold text-sm bg-emerald-600 text-white hover:bg-emerald-700 shadow-md shadow-emerald-200 transition-all flex items-center gap-2"
          >
            <CheckCircle size={18} />
            Process Payment
          </button>
